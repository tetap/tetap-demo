...
import { LocaleConfigs, parseProps, resolveLocaleConfigs } from '@ruabick/utils'
...
export function genApiDoc(): Plugin {
  ...
  return {
    ...
    transform: async (code, id) => {
      ...
      const baseDir = frontmatter.realPath ? dirname(frontmatter.realPath) : __dirname
      ...
      for (const block of blocks) {
        if (!block.match(API_REG)) {
          ...
          replacedBlocks.push(await getApiMarkdown(block, localeConfigs, baseDir))
        }
      }
...
async function getApiMarkdown(apiComponent: string, localeConfigs: LocaleConfigs, baseDir: string) {
