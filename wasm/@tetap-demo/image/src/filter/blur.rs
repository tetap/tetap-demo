...
#[wasm_bindgen]
#[allow(dead_code)]
pub fn stack_blur(data: &mut [u8], width: u32, height: u32, strength: u8) {
    let radius = if strength <= 0 {
        0.0
    ...
    let div = (radius + radius + 1.0) as u32;
    let w4 = width << 2;
    let width_minus_1 = width - 1;
    let height_minus_1 = height - 1;
    let radius_plus_1 = radius + 1.0;
    let sum_factor = radius_plus_1 * (radius_plus_1 + 1.0) / 2.0;
}
